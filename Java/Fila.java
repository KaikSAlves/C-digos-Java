public class Fila {
    ...
    public void organizarPorPrioridade() {
        ...
        for (int i = 0; i < this.tamanho; i++) {
            if (busca(i).getPrioridade() == 1) {
                if (i != 0) {
                    Senha senha = busca(i);
                    remove(i);
                    adiciona(indicePrioridade, senha);
                }
                indicePrioridade++;
            }
        }
    ...
    public int busca(Senha senha) {
        for(int i = 0; i < this.tamanho; i++){
            if(senha.getNum() == this.elementos[i].getNum()){
                return i;
            }
        }
    ...
    public boolean contem(int num) {
        if (estaVazia()) {
            ...
            for (int i = 0; i < this.tamanho; i++) {
                if (this.elementos[i].getNum() == num) {
                    return true;
                }
            }
        }
